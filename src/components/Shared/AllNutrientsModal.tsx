...
export const AllNutrientsModal = ({
  ...
  const categorizedNutrients = categorizeNutrients(data?.nutrition);
  ...
  // Support both ingredient (name) and recipe (title) data structures
  const displayName = data?.name?.[language] || data?.title?.[language] || '';
  const displayImage = data?.image?.url || data?.thumbnail?.url;

  if (!showAllNutrients) return null;

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
      <Card className="w-full max-w-4xl max-h-[90vh] overflow-hidden bg-white shadow-2xl border-0 relative top-5 lg:top-10">
              ...
              <div className="flex items-start lg:items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm md:text-lg font-bold text-gray-900">
                    {displayName} - {language === 'ar' ? 'جميع العناصر الغذائية' : 'All Nutrients'}
                  </h3>
                      ...
                      {Object.entries(nutrients as any).map(([key, data]: [string, any]) => (
                        ...
                        >
                          ...
                          <div className="text-right">
                            <span className="text-primaryColor text-xs md:text-base block">
                            ...
                            {data?.percentage !== undefined && data.percentage !== null && (
                              <div
                                className={`px-2 py-1 rounded text-xs md:text-base mt-1 ${getNutritionBgColor(
                                  data.percentage,
