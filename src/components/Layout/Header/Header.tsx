...
const MobileSearchOverlay: React.FC<{
  ...
  return (
    <div className="lg:hidden fixed inset-0 bg-white z-50 safe-area-inset-top">
      {/* Search Header */}
      <div
        className={`flex items-center justify-between px-4 py-3 border-b border-gray-200 ${isRTL ? 'flex-row-reverse' : ''}`}
      >
        <button
          onClick={onClose}
        ...
        <span className="text-lg font-semibold text-gray-900">{isRTL ? 'بحث' : 'Search'}</span>
...
export const Header: React.FC = (): JSX.Element => {
  ...
  const handleNavClick = (item: NavItem) => {
    if ((item.name === 'Profile' || item.name === 'الملف الشخصي') && !currentUser) {
  ...
  return (
    <>
      ...
      <header className="w-full h-20 bg-white shadow-header sticky -top-1 z-[50] hidden lg:flex">
        ...
        >
          {/* Logo */}
          <Link to={getLocalizedPath('/')} className="w-[95px] h-[71px] flex-shrink-0">
          ...
          <div className={`flex items-center  gap-4 lg:gap-5 xl:gap-7`}>
            ...
            <div className="relative" ref={searchRef}>
              ...
              {isSearchOpen && (
                <div
                  className={`absolute top-full mt-[26px] ${isRTL ? 'left-0' : 'right-0'} w-[700px] xl:w-[800px] h-[60px] xl:h-[70px] z-[9999]`}
                >
