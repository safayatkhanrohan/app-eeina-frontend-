...
const ProfileViewGoals = () => {
  ...
  const { data: goalData, isLoading: isGoalsLoading } = useGetGoalsQuery({
    status: "In Progress",
  });
  ...
  const handleUpdateMealProgress = async (
  ...
  if (isGoalsLoading || isMealPlanLoading)
    return <Loader className="bg-white" />;

  const nutrition = goalData?.data[0]?.targetNutrition;

  console.log("goalData", goalData);
  if (!goalData?.data || goalData.data.length === 0) return <NoGoalsMessage />;
    ...
    <div className="flex flex-col gap-6">
      ...
      {nutrition && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <Card className="p-4 flex flex-col gap-2 items-center justify-center text-center hover:shadow-md transition-shadow">
            ...
            <div>
              <p className="text-xs text-gray-500 font-medium">
                {t.goalSetup?.label_calories || "Calories"}
              </p>
              <h3 className="text-lg font-bold text-gray-900">
                {nutrition.calories}
                <span className="text-xs font-normal text-gray-400">
                  {t.goalSetup?.kcal || "kcal"}
                </span>
              </h3>
            </div>
          </Card>
          <Card className="p-4 flex flex-col gap-2 items-center justify-center text-center hover:shadow-md transition-shadow">
            ...
            <div>
              <p className="text-xs text-gray-500 font-medium">
                {t.goalSetup?.label_protein || "Protein"}
              </p>
              <h3 className="text-lg font-bold text-gray-900">
                {nutrition.protein}{" "}
                <span className="text-xs font-normal text-gray-400">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="flex flex-col gap-5 h-full">
          <ProfileWeightCard
            currentWeight={
              goalData?.data[0].weightLog?.[
                goalData?.data[0].weightLog?.length - 1
              ]?.weight || goalData?.data[0].currentWeight
            }
          ...
          <Card className="p-4 flex flex-row justify-between items-center gap-4">
            <div className="flex flex-col">
              <h2 className="font-semibold text-gray-900">
                {t.goals.mealPlannerTitle}
              </h2>
              <p className="text-xs text-gray-500">{t.goals.mealPlannerDesc}</p>
            </div>
            <Link to="/planner">
