    ...
    if (goalData) {
      ...
      if (targetWeight !== currentWeight) {
        ...
        if (Math.abs(caloriesDifference) > tdee * 0.33) {
          setWarning(true);
          setWarningMessage(
            `${targetWeight > currentWeight ? "gaining" : "losing"} ${Math.abs(
              targetWeight - currentWeight
            )} kg in ${Math.floor(
              (new Date(endDate).getTime() - new Date(startDate).getTime()) /
                (1000 * 60 * 60 * 24 * 7)
            )} weeks requires a significant calorie ${
      ...
      <div className="flex flex-col gap-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="p-0 border-none shadow-none md:border md:shadow-sm md:p-4">
            <h3 className="font-semibold text-lg mb-4 px-1">{t.goals.WeightHistory}</h3>
            <WeightLog
